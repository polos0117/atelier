...
(function (root) {
  ...
  function drawBody(canvas, value, gender) {
    ...
    var thigh = Math.max(7, hip * (female ? .71 : .66) * f.thickness);
    var ankle = Math.max(4.2, thigh * (female ? .40 : .46));
    var neck = female ? 4.2 : 5.8, shoulderDrop = female ? 7 : 4;
    ...
    /* 부드러운 마네킹 실루엣. FIG의 폭 차이는 그대로 쓰고 관절만 곡선으로 잇는다. */
    ctx.save();
    ...
    ctx.beginPath(); ctx.arc(cx, top + 10.5, 10.5, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
