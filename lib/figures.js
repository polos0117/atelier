...
(function (root) {
  'use strict';
  /* 표는 부를 때 찾는다. 읽어들이는 차례가 어긋나도 조용히 죽지 않게 —
     예전에 이 파일이 lib/toolkit-spec.js 보다 먼저 실행되도록 놓여,
     도식을 펼치는 순간 화면이 통째로 멈춘 적이 있다 */
  function spec() {
  ...
  function bodySVG(k, gender){
    const f=spec().BODY_FIG[k];if(!f)return'';
  ...
  function hairSVG(k){
    const f=spec().HAIR_FIG[k];if(!f)return'';
