...
## 12. [ALREADY USED] 를 프롬프트에서 뺐다

11 절과 같은 종류인데 더 나빴다. 이 블록은 이 기체로 이미 뽑은 조합을 늘어놓고
"이것들과 다르게 하라" 고 시켰는데, 목록에 들어가던 것이 **내부 열쇠를 밑줄만
바꾼 것**이었다.

    const t = r.ex.replace(/_/g,' ')      open_shirt_lingerie → "open shirt lingerie"

사람이 읽으라고 지은 말도 아니고 화면에 뜨는 이름도 아닌, 코드가 쓰는 이름이
그대로 나갔다. 콜라주는 `Poolside Bar (poolside bar)` 처럼 이름과 열쇠를 나란히
적기까지 했다. 기록이 쌓인 기체에서는 이 목록이 예순 줄을 넘는다.

콜라주·단일 두 자리에서 뺐다.

### 회피 기능은 살아 있다

이미 쓴 것을 피하는 일은 **프롬프트가 아니라 뽑기 단계**에서 한다. `freshPool` 이
"이미 쓴 카테고리는 빼고 뽑기" 를 보고 후보에서 제외하므로, 이력을 프롬프트에
적지 않아도 겹치지 않는다. 화면의 쓴 컷 목록도 그대로다.

### 죽은 스위치를 함께 치웠다

콜라주의 "이미 복사한 조합을 프롬프트에서 피하기" 는 하는 일이 이 블록 하나뿐이라
**아무 일도 안 하는 체크박스가 되었다.** 이름표가 하는 일을 말하고 있으므로
남겨 두면 거짓말이 된다. 체크박스와 저장·복원·상태줄·리스너를 함께 뺐다.

단일 컷의 것은 남겼다 — 그쪽은 랜덤 카테고리 뽑기에도 걸려 있어 여전히 일한다.
다만 문구가 "프롬프트 + 랜덤 카테고리" 였으므로 실제와 맞게 고쳤다.

    이미 복사한 조합 피하기 — 프롬프트 + 랜덤 카테고리
    → 이미 쓴 카테고리는 랜덤에서 빼기

### 확인

쓴 컷 기록 62 가지가 쌓인 기체(건담 사바냐)로 열어 재었다. 콜라주·단일 어느 쪽에도
`[ALREADY USED]` 가 없고 내부 열쇠 흔적도 없다. 콜라주 12,002 자 · 단일 8,896 자.
