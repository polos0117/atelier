...
def crosscheck(mech, pilot):
    ...
    고치지는 않는다 — 색은 build-gge-tags.py 가 맡는다. 다만 그 스크립트는
    공식이 색을 말하지 않은 자리에 우리 녹색·청색을 남겨 두므로(공식에는
    흰·빨강·검정·금 넷뿐이라 표현할 칸이 없다) 여기 "색 어긋남" 은 비어 있는
    것이 정상이고, 뭔가 뜬다면 둘이 갈렸다는 뜻이다. 역할·계통은 여전히
    손으로 볼 몫이라 어디를 봐야 하는지만 알려 준다."""
