...
const html=fs.readFileSync('sam.html','utf8');
const code=html.match(/<script>\s*([\s\S]*?)<\/script>/)[1].replace(/^\s*\(function\(\)\{/, '').split('buildTiers(C);buildHelp();buildDex();')[0];
...
function step(c){if(c.queue.length)c.queue.shift()();else if(c.myTurn)c.myPick(c.legalSet(c.mine,c.openIdx())[0]);else if(c.banQ[0]==='me')c.myBan(c.banCand.findIndex(e=>!e.by));else throw Error('stalled')}
function invariant(c){
 ...
 for(const t of [c.mine,c.foe,c.foe2].filter(Boolean))assert(t.length<=11);
...
for(const players of [2,3])for(const mode of [0,1])for(const ex of [false,true])for(const pk of [null,'w194','gd200','jb208','end263','mx']){
 c=runtime({},played+1);c.PLAYERS=players;c.MODE=mode;c.exhaustOn=ex;c.PACK2=pk?c.PACKS[pk]:null;c.banOn=played%2===0;c.start();assert.equal(c.PACK,players===3?9:8);
...
for(const phase of ['ban','draft','next']){
 const storage={};c=runtime(storage);c.banOn=phase==='ban';c.PLAYERS=3;c.MODE=1;c.start();
 for(let i=0;JSON.parse(storage[c.GAME_KEY]).phase!==phase&&i<30;i++)step(c);
