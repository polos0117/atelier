...
- `node tests/figures.cjs`: BODY_FIG·HAIR_FIG의 모든 값을 여성·남성 체형 및 헤어 Canvas 이미지로 그릴 수 있는지와 이미지 선택 연결을 검사한다.
- `node tests/workspace-theme.cjs`: 미드나이트·화이트 베이스·아크엔젤 공통 선택, 저장값 정규화, 브라우저 테마 색상과 가시 높이 계산을 검사한다.
